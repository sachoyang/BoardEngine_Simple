...
> 최종 업데이트: Session 23 (Label 컴포넌트 + SetObjectText API 추가 — 오브젝트 위 텍스트. Unity TextMesh / Godot Label 모티브)
> Session 22 (RestartGame API 추가 — 재대국/씬 리로드. Godot reload_current_scene 모티브)
>
...
│   ├── Label.h / .cpp          ← 오브젝트 위 텍스트 컴포넌트 (ImGui 오버레이 렌더)
...
Component (추상 기반)
  ...
  ├── SpriteRenderer    — 텍스처 핸들, visible 플래그, colorTint
  ├── Label             — 오브젝트 위 텍스트(글자·색·크기). ImGui 오버레이로 렌더
...
```
LoadScene(path)
  [0] m_scenePath = path            ← RestartGame() 이 다시 로드할 경로 기억
...
public:
    ...
    // ── 재대국 / 씬 리로드 ───────────────────────────────────────────────
    // 마지막으로 로드한 씬(scene.json)을 처음 상태로 다시 로드하고,
    // 로드된 GameLogic 의 OnLoad() 를 재호출한다 — 보드게임 "Rematch" 한 줄 구현.
    // Godot 의 SceneTree.reload_current_scene() 에서 착안했다.
    // 실제 리로드는 Deferred(프레임 끝)로 처리되므로 Update/OnObjectClicked
    // 순회 도중에 호출해도 안전하다. 호출 직후에는 오브젝트 포인터를 더 쓰지 말고 return 하라.
    virtual void RestartGame() = 0;

    // ── 오브젝트 위 텍스트 (Label) ───────────────────────────────────────
    // 오브젝트 위치(Transform 중심)에 글자를 겹쳐 표시한다. Label 컴포넌트를 자동 부착한다.
    // 스프라이트만으로 종류 구분이 어려운 게임에서 사용 — 예: 체스 말에 "N"(나이트) 표시.
    // text 가 비면 라벨을 지운다. (r,g,b) 는 글자 색(0~1, 기본 흰색).
    // 엔진이 글자 밝기에 맞춰 대비 아웃라인을 자동으로 그려 어떤 배경에서도 읽힌다.
    // Unity TextMesh / Godot Label 모티브.
...
| `RestartGame()` | `void` | 씬 초기화 후 재대국 (게임 종료 → 재시작) |
| `SetObjectText(obj, text, r,g,b)` | `void` | 오브젝트 위 글자 표시 (체스 말 종류 등) |
...
**RestartGame (재대국) — Deferred Reload 메커니즘**

`RestartGame()` 은 즉시 리로드하지 않고 `m_pendingRestart` 플래그만 세웁니다.
프레임 끝(Deferred Destroy Flush 직후)에서 씬을 다시 로드하고 `OnLoad` 를 재호출합니다.
`Destroy` 와 같은 이유로 — 입력·오브젝트 순회가 모두 끝난 뒤 처리하므로
`OnObjectClicked` 안에서 호출해도 포인터 무효화 크래시가 없습니다.

```
Render() Update 블록
  ├── GameLogic::OnObjectClicked()   ← RestartGame() 호출 → m_pendingRestart = true
  ├── [Deferred Destroy Flush]
  └── [Deferred Restart]             ← LoadScene(m_scenePath) + gameLogic->OnLoad(...)
```

- `m_scenePath` : 마지막으로 `LoadScene()` 에 전달된 경로(기본 `"assets/scene.json"`).
  STANDALONE 자동 로드·에디터 Load Scene·MakeSDK 배포판 모두 이 값을 갱신하므로,
  `RestartGame()` 은 항상 "현재 플레이 중인 씬"을 정확히 복원합니다.
- 씬을 리로드하면 모든 `SpriteRenderer::colorTint` 가 저장된 원본값으로 돌아가므로
  (오목의 돌 착색 등) 보드가 완전히 초기 상태로 돌아갑니다.

```cpp
// 예시: 오목 — 게임 종료 후 아무 곳이나 클릭하면 재대국
void OnObjectClicked(GameObject* obj) override
{
    if (m_gameOver)
    {
        m_api->RestartGame();  // 엔진이 씬 초기화 + OnLoad 재호출
        return;                // 이후 obj/objects 접근 금지
    }
    // ... 일반 착수 로직 ...
}
```

**SetObjectText / Label — 오브젝트 위 텍스트 렌더 메커니즘**

`SetObjectText(obj, text, r,g,b)` 는 `obj` 에 `Label` 컴포넌트를 자동 부착(없으면 생성)하고 글자·색을 설정합니다.
실제 그리기는 `Engine::RenderObjectLabels()` 가 매 프레임 **ImGui 프레임 안에서** 수행합니다.

```
Render() ImGui 블록
  ├── (메뉴/인스펙터 등 에디터 UI)
  ├── 게임 상태 텍스트 오버레이 (SetGameStatusText)
  └── RenderObjectLabels()
        └── Label 을 가진 오브젝트마다:
              world(tr->x,y) → screen = (world - camera) * zoom  로 투영
              밝기 대비 아웃라인 4방향 + 본문 글자를 배경 드로우리스트에 AddText
```

- **D3D12 파이프라인 무변경.** 스프라이트는 D3D12 로, 글자는 ImGui 드로우리스트로 그리므로 셰이더/CB/정점 버퍼를 건드리지 않습니다. 리스크·비용이 낮습니다.
- ImGui 프레임은 STANDALONE_MODE 에서도 항상 호출되므로 배포판에서도 라벨이 보입니다.
- 폰트는 ImGui 기본 폰트(ASCII 위주). 글리프가 없는 문자는 표시되지 않으니 체스는 알파벳(P·N·B·R·Q·K)을 씁니다.

```cpp
// 예시: 체스 — 색칠된 칸 + 글자로 말을 표현 (이미지 에셋 불필요)
GameObject* p = m_api->Instantiate(name, x, y);
m_api->AddSpriteRenderer(p, "assets/tile.png");
p->GetComponent<SpriteRenderer>()->colorTint = TINT_WHITE_PIECE; // 팀 색
...
| `RestartGame` 호출 후 | Deferred 이므로 즉시 리로드 안 됨. 호출 직후 `objects`/`clickedObj` 포인터 접근 금지, 바로 return |
| `SetObjectText` 렌더 순서 | Label 은 ImGui 배경 드로우리스트로 그려져 **스프라이트 위**에 표시됨. 폰트는 ImGui 기본 폰트라 ASCII 위주(체스는 P·N·B·R·Q·K 로 충분) |
| `SetObjectText` 위치/크기 | Transform 중심에 정렬, 카메라 이동·줌을 따라감. 크기는 `Label::size`(zoom 1.0 기준 px) |
...
### 새 IEngineAPI 함수 추가 시 (DLL 경계 API)

- [ ] `Engine\Engine\IEngineAPI.h` 에 순수 가상 함수 + 주석 추가 (맨 끝에 append — vtable 순서 유지)
- [ ] `BoardEngine_SDK\include\IEngineAPI.h` 사본에도 **동일하게** 추가 (MakeSDK.ps1 재실행 전까지 수동 동기화)
- [ ] `Engine.h` 에 `override` 선언 추가
- [ ] `Engine.cpp` 에 구현 추가. 순회 중 상태를 바꾸는 API 는 **Deferred**(플래그/큐 → 프레임 끝 처리)로 구현
- [ ] Engine 과 GameLogic 을 **함께 재빌드** (ABI 일치 필수 — vtable 이 바뀜)
- [ ] 이 문서의 **IEngineAPI 명세**(섹션 13) 인터페이스·요약표·주의사항 업데이트
